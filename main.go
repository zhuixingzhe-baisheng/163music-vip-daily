...
func autoTasks(userData types.LoginStatusData, data utils.RequestData) error {
	...
	if config.ShowVipTaskList {
		showVipTaskList(userData, data)
	}
	
	if config.VipSongLike {
		err := vipSongLikeTask(userData, data)
		if err != nil {
			log.Errorln(err)
		}
	}
	...
	if config.VipListen {
		err := vipListenTask(userData, data)
		if err != nil {
			log.Errorln(err)
		}
	}
	...
	if strings.Contains(userDetail.CurrentExpert.RoleName, "网易音乐人") {
		...
		err = sendEventWithDelete(userData, data)
		if err != nil {
			log.Println(err)
		}
		...
		log.Printf("[%s] 音乐人任务执行完成", userData.Profile.Nickname)
	}
	if strings.Contains(userDetail.CurrentExpert.RoleName, "网易音乐人") {
		artistDetail, err := api.GetArtistHomepage(data, int64(userDetail.Profile.ArtistId))
		parseCircleID(artistDetail)
...
func vipSignTask(userData types.LoginStatusData, data utils.RequestData) error {
	vipStat, err := api.GetVipInfo(data)
	if err != nil || vipStat.Data.RedVipLevel == 0 {
		return nil
	}
	type req struct {
	...
	body, _ := json.Marshal(req{TaskId: "14441", TaskCode: "vipgrowth_sign"})
	var opt utils.EapiOption
	opt.Path = "/api/vipnewcenter/app/level/task/sign/get"
	opt.Url = "https://music.163.com/api/vipnewcenter/app/level/task/sign/get"
	opt.Json = string(body)
	res, _, err := utils.ApiRequest(opt, data)
	if err != nil {
		return nil
	}
	var r map[string]interface{}
	json.Unmarshal([]byte(res), &r)
	if c, _ := r["code"].(float64); int(c) == 200 {
		reward, _ := api.VipTaskReward(data, "14441")
		if reward.Code == 200 && reward.Data.Result {
			log.Printf("[%s] 黑胶会员签到成功，获得成长值 +10", userData.Profile.Nickname)
			pushMsg += fmt.Sprintf("\n[%s] 黑胶会员签到成功，获得成长值 +10", userData.Profile.Nickname)
		}
	}
	return nil
}

func showVipTaskList(userData types.LoginStatusData, data utils.RequestData) error {
	vipStat, _ := api.GetVipInfo(data)
	if vipStat.Data.RedVipLevel == 0 {
		return nil
	}
	taskList, err := api.GetVipTasksList(data)
	if err != nil {
		return nil
	}
	log.Printf("[%s] = VIP 任务列表 =", userData.Profile.Nickname)
	for _, tg := range taskList.Data.TaskList {
		for _, t := range tg.TaskItems {
			name := t.Name
			if name == "" {
				name = t.Description
			}
			if name != "" {
				log.Printf("[%s] %s | 进度:%d/%d | 成长值:+%d", name, t.CurrentProgress, t.TargetWorth, t.GrowthPoint)
			}
		}
	}
	return nil
...
func vipSongLikeTask(userData types.LoginStatusData, data utils.RequestData) error {
	vipStat, _ := api.GetVipInfo(data)
	if vipStat.Data.RedVipLevel == 0 {
		return nil
	}
	taskList, _ := api.GetVipTasksList(data)
	var needTask bool
	for _, tg := range taskList.Data.TaskList {
		for _, t := range tg.TaskItems {
			name := t.Name
			...
			if strings.Contains(name, "收藏") && t.CurrentProgress < t.TargetWorth {
				needTask = true
			}
		}
	}
	if !needTask || len(config.VipSongLikeIDs) == 0 {
		return nil
	}
	for _, songID := range config.VipSongLikeIDs {
		body, _ := json.Marshal(map[string]interface{}{"trackId": songID, "like": true})
		var opt utils.EapiOption
		opt.Path = "/api/song/like"
		opt.Url = "https://music.163.com/api/song/like"
		opt.Json = string(body)
		utils.ApiRequest(opt, data)
		time.Sleep(2 * time.Second)
	}
	time.Sleep(3 * time.Second)
	updatedList, _ := api.GetVipTasksList(data)
	for _, tg := range updatedList.Data.TaskList {
		for _, t := range tg.TaskItems {
			if strings.Contains(t.Name, "收藏") && t.CurrentProgress >= t.TargetWorth {
				reward, _ := api.VipTaskReward(data, t.TaskId)
				if reward.Code == 200 && reward.Data.Result {
					log.Printf("[%s] 领取成长值 +%d", userData.Profile.Nickname, t.GrowthPoint)
					pushMsg += fmt.Sprintf("\n[%s] VIP 收藏任务 +%d", userData.Profile.Nickname, t.GrowthPoint)
				}
			}
		}
	}
...
func vipListenTask(userData types.LoginStatusData, data utils.RequestData) error {
	vipStat, _ := api.GetVipInfo(data)
	if vipStat.Data.RedVipLevel == 0 {
		return nil
	}
	...
	for _, tg := range taskList.Data.TaskList {
		for _, t := range tg.TaskItems {
			...
			if strings.Contains(name, "听歌") && t.CurrentProgress < t.TargetWorth {
				needTask = true
			}
		}
	}
	if !needTask || config.VipPlaylistID == 0 {
		return nil
	}
	pl, _ := api.GetPlaylistDetail(data, config.VipPlaylistID)
	var songIDs []int
	for _, tr := range pl.Playlist.Tracks {
		songIDs = append(songIDs, tr.Id)
		if len(songIDs) >= 10 {
			break
		}
	}
	...
	for _, id := range songIDs {
		songs = append(songs, song{SongId: fmt.Sprintf("%d", id), PlayType: 3, SourceId: fmt.Sprintf("%d", config.VipPlaylistID), Time: 240})
	}
	dataJson, _ := json.Marshal(songs)
	body, _ := json.Marshal(map[string]string{"data": string(dataJson)})
	var opt utils.EapiOption
	opt.Path = "/api/feedback/weblog"
	opt.Url = "https://music.163.com/weapi/feedback/weblog"
	opt.Json = string(body)
	res, _, _ := utils.ApiRequest(opt, data)
	var r map[string]interface{}
	json.Unmarshal([]byte(res), &r)
	if c, _ := r["code"].(float64); int(c) == 200 {
		time.Sleep(3 * time.Second)
		upList, _ := api.GetVipTasksList(data)
		for _, tg := range upList.Data.TaskList {
			for _, t := range tg.TaskItems {
				name := t.Name
				if name == "" {
					name = t.Description
				...
				if strings.Contains(name, "听歌") && t.CurrentProgress >= t.TargetWorth {
					...
					if reward.Code == 200 && reward.Data.Result {
						log.Printf("[%s] 领取成长值 +%d", userData.Profile.Nickname, t.GrowthPoint)
						pushMsg += fmt.Sprintf("\n[%s] VIP 听歌任务 +%d", userData.Profile.Nickname, t.GrowthPoint)
					}
				}
			}
		}
...
// sendEventWithDelete 发送动态并在下次执行时删除上一次的动态
func sendEventWithDelete(userData types.LoginStatusData, data utils.RequestData) error {
	defer func() {
		err := recover()
		if err != nil {
			log.Errorln(err)
	...
	for i := 0; i < 1; {
		...
		// 读取上次动态 ID
		var lastEventId int
		if dataFromFile, err := os.ReadFile(".last_event_id"); err == nil {
			fmt.Sscanf(string(dataFromFile), "%d", &lastEventId)
		}
		
		// 如果有上次动态 ID，先删除
		if lastEventId != 0 {
			log.Printf("[%s] 正在删除上次动态 (ID: %d)", userData.Profile.Nickname, lastEventId)
			delResult, err := api.DelEvent(data, lastEventId)
			if err != nil {
				log.Printf("[%s] 删除上次动态失败：%s", userData.Profile.Nickname, err)
			} else if delResult.Code == 200 {
				log.Printf("[%s] 删除上次动态成功 (ID: %d)", userData.Profile.Nickname, lastEventId)
			} else {
				log.Printf("[%s] 删除上次动态失败，代码：%d, 原因：%s", userData.Profile.Nickname, delResult.Code, delResult.Message)
			}
		}
		...
		if sendResult.Code == 200 {
			log.Printf("[%s] 发送动态成功，动态 ID: %d, 内容：\"%s\"", userData.Profile.Nickname, sendResult.Event.Id, msg)
			i++
			
			// 保存当前动态 ID 到文件
			os.WriteFile(".last_event_id", []byte(fmt.Sprintf("%d", sendResult.Event.Id)), 0644)
			
			// 延时（不删除当前动态，留到下次执行时删除）
			if config.EventSendConfig.LagConfig.LagBetweenSendAndDelete {
				randomLag := eventLag.Get()
				if randomLag != 0 {
					log.Printf("[%s] 延时 %d 秒", userData.Profile.Nickname, randomLag)
